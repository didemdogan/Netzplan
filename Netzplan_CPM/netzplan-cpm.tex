...
{\huge
    ...
    \item<only@+> {Schritt wie gewohnt.}
    \item<only@+> {Schritt wie gewohnt.}
    \item<only@+> {Zeichne den kritischen Pfad ein.}
    \item \alert<+> {}
}
...
\only<1>{\begin{alertblock}{Merke!}
    Die Nummerierungen der Kreise haben nichts mit der Nummerierung der Vorgänge zu tuen. Kreisnummer wird beliebig eingesetzt.
\end{alertblock}
}
